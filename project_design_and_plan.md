...
语义相似度路由 (Semantic Similarity Routing): 对于更复杂的任务，此策略将用户的查询（或任务描述）进行嵌入，然后与一个预定义的“任务嵌入向量”列表进行相似度计算。例如，预定义“项目摘要”、“面试答案评估”、“简历与JD对比”等任务的嵌入。通过找到最相似的任务向量来决定路由，这个过程避免了为路由本身进行一次 LLM 调用，从而节省了成本和延迟 。   
